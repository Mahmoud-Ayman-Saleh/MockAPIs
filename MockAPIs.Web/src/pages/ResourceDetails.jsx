...
export function ResourceDetails() {
  ...
  useEffect(() => {
    const fetchMeta = async () => {
      ...
      try {
        const projRes = await api.get(`/api/Project/${projectId}`);
        if (projRes.data) {
          setProjectToken(projRes.data.token || '');
          const resObj = projRes.data.resources?.find(
            (r) => r.id.toLowerCase() === resourceId.toLowerCase()
          );
          if (resObj) {
            setResourceSlug(resObj.slug || '');
          }
        }
      } catch (err) {
        console.error('Failed to fetch project/resource metadata:', err);
      }
    };
    fetchMeta();
